...
internal sealed class ParserServiceClient(HttpClient httpClient) : IParserServiceClient
{
    public async Task<ParserProxyListResponse> ListProxiesAsync(bool? enabledOnly, CancellationToken ct)
    {
        ...
        return (await response.Content.ReadFromJsonAsync<ParserProxyListResponse>(ct))!;
    }

    public async Task<ParserProxyDto> CreateProxyAsync(CreateParserProxyRequest request, CancellationToken ct)
    {
        var response = await httpClient.PostAsJsonAsync("api/proxies", request, ct);
        await EnsureSuccess(response, ct);
        return (await response.Content.ReadFromJsonAsync<ParserProxyDto>(ct))!;
    }

    public async Task DeleteProxyAsync(int id, CancellationToken ct)
    {
        var response = await httpClient.PostAsJsonAsync("api/proxies/delete", new ParserProxyIdRequest(id), ct);
        await EnsureSuccess(response, ct);
    }
        ...
        string? status, string? source, int? limit, int? offset, CancellationToken ct)
    {
        ...
        await EnsureSuccess(response, ct);
        return (await response.Content.ReadFromJsonAsync<ParserCollectionTaskListResponse>(ct))!;
    }
    ...
    public async Task<ParserCollectionTaskStatusResponse> GetTaskStatusAsync(Guid taskId, CancellationToken ct)
    {
        ...
        await EnsureSuccess(response, ct);
        return (await response.Content.ReadFromJsonAsync<ParserCollectionTaskStatusResponse>(ct))!;
    }
    ...
    {
        var response = await httpClient.PostAsJsonAsync("api/collection-tasks", request, ct);
        await EnsureSuccess(response, ct);
        return (await response.Content.ReadFromJsonAsync<CreateParserCollectionTaskResponse>(ct))!;
    }

    private async Task<ParserProxyDto> PostProxyAction(string path, int id, CancellationToken ct)
    {
        var response = await httpClient.PostAsJsonAsync(path, new ParserProxyIdRequest(id), ct);
        await EnsureSuccess(response, ct);
        return (await response.Content.ReadFromJsonAsync<ParserProxyDto>(ct))!;
    }
