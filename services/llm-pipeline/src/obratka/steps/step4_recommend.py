...
async def generate_recommendations(
    ...
    *,
    ...
    sys_prompt = (
        ...
        # [VERIFY-CTX] временная проверка доставки контекста — удалить вместе с
        # блоком в user-промпте после подтверждения.
        "[VERIFY-CTX] В самом начале summary добавь строку вида "
        "'CTX: category=<...>; subcategory=<...>; additional=<...>' с фактически "
        "полученными значениями бизнес-контекста (или «—», если поле не задано). "
        "В evidence каждой рекомендации первым элементом поставь тег "
        "'[Категория: <подкатегория, иначе категория, иначе —>]', а далее — минимум "
        "одну дословную цитату отзыва, на которой основана рекомендация.\n\n"
