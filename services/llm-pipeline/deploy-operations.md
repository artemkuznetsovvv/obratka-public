...
VPS: `193.233.217.223`, пользователь `deploy`, проектная директория `~/llm-pipeline/`.
...
> по docker-DNS `http://obratka-llm-worker:8000`, сообщения принимает через
...
На **локальной машине** (WSL Ubuntu):
...
rsync -avz --delete \
    ...
    ./ deploy@193.233.217.223:~/llm-pipeline/
```
...
LLM-pipeline собирается **из проекта Processing Gateway**, потому что compose-override
`docker-compose.with-llm-pipeline.yml` лежит там и указывает `context: ../llm_pipline`.
...
docker compose \
    ...
    -f docker-compose.with-llm-pipeline.yml \
    build llm-pipeline
...
docker compose logs -f obratka-llm-worker    # Ctrl+C когда увидели
                                              # "LLM service listening on llm.requests"
                                              # + "Uvicorn running on http://0.0.0.0:8000"
```

`docker compose up -d llm-pipeline` пересоздаёт **только** контейнер `obratka-llm-worker`.
RabbitMQ, MinIO, Postgres, Seq, PG-worker — не трогаются. SQLite job-state
(`data/job_state.sqlite`) переживает рестарт, потому что лежит в named volume
`llm-pipeline-state`.

> **Tip:** чтобы не таскать три `-f` флага каждый раз — см. §6.1 алиасы.
...
|----------------|---------|
| `docker-compose.with-llm-pipeline.yml` | `docker compose ... up -d llm-pipeline` (применит diff без ребилда) |
...
```bash
docker tag obratka-llm-worker:dev obratka-llm-worker:backup-$(date +%F)
```
...
```bash
docker tag obratka-llm-worker:backup-2026-05-11 obratka-llm-worker:dev
docker compose \
    -f docker-compose.yml \
    -f docker-compose.full.yml \
    -f docker-compose.with-llm-pipeline.yml \
    up -d --force-recreate llm-pipeline
```

Плюс `git` локально — всегда можно `git checkout <commit>` + `rsync` старую версию кода.

**Внимание:** при откате `data/job_state.sqlite` **не** откатывается — это persistent volume.
Если новая версия успела записать туда несовместимый формат — придётся почистить
(см. §3.3).
...
docker compose ps obratka-llm-worker

# 2. На старте нет exception, AMQP подписался, HTTP стартанул
docker compose logs --tail=50 obratka-llm-worker
# Ожидание:
#   - "LLM service listening on llm.requests"
#   - "Uvicorn running on http://0.0.0.0:8000"
...
docker compose exec obratka-llm-worker \
    python -c "import urllib.request; print(urllib.request.urlopen('http://localhost:8000/health/live').read())"
# Ожидание: b'{"status":"alive"}'

# 4. Очередь llm.requests реально имеет подписчика
docker compose exec rabbitmq rabbitmqctl list_queues name consumers messages_ready
...
Если контейнер в `Restarting` — `docker compose logs obratka-llm-worker` покажет причину
(чаще всего: пустой `OPENROUTER_API_KEY` или недоступный RabbitMQ — см. §4).
...
docker compose \
    ...
    -f docker-compose.full.yml \
    -f docker-compose.with-llm-pipeline.yml \
    up -d llm-pipeline       # ВАЖНО: именно up -d, не restart — env_file
                              # перечитывается только при пересоздании контейнера

docker compose logs -f obratka-llm-worker | head -30
...
```bash
docker compose logs obratka-llm-worker | grep -E "cost|tokens" | tail -50
```
...
LLM-pipeline хранит три набора данных. Все **внутри docker-volumes**, не в коде:

| Данные | Где | Volume |
|--------|-----|--------|
| `/status/{jobId}` — состояние job-ов (stage, progress, started_at) | `/app/data/job_state.sqlite` | `llm-pipeline-state` |
| Логи loguru (rotating) | `/app/logs/*.log` | `llm-pipeline-logs` |
| QA-выходы (когда `OBRATKA_QA_ENABLED=true`) | `/app/qa_outputs/<job_id>/` | bind-mount `./llm-qa-outputs/` |
...
```bash
docker compose exec obratka-llm-worker ls -la /app/logs/
docker compose exec obratka-llm-worker tail -n 200 /app/logs/obratka.log
```

Или через docker:

```bash
docker compose logs --tail=200 obratka-llm-worker          # последние 200 строк
docker compose logs --since=10m obratka-llm-worker         # за последние 10 минут
docker compose logs -f obratka-llm-worker | grep -E "ERROR|WARN"
```
...
docker compose stop obratka-llm-worker
docker volume rm processing-gateway_llm-pipeline-state   # точное имя volume — см. docker volume ls
docker compose \
    -f docker-compose.yml \
    -f docker-compose.full.yml \
    -f docker-compose.with-llm-pipeline.yml \
...
ротация не работает и volume распух:

```bash
docker compose exec obratka-llm-worker sh -c 'find /app/logs -name "*.log*" -mtime +7 -delete'
docker compose exec obratka-llm-worker du -sh /app/logs
```

### 3.5. QA-выходы

Если включён `OBRATKA_QA_ENABLED=true` (см. §4) и `qa_outputs/` bind-mount'нут —
файлы доступны прямо на хосте:

```bash
ls -la ~/processing-gateway/llm-qa-outputs/
cat ~/processing-gateway/llm-qa-outputs/<job_id>/output_summary.json | jq
```
...
В `docker-compose.with-llm-pipeline.yml` (на VPS — `~/processing-gateway/`) раскомментировать:
...
volumes:
  ...
  - ./llm-qa-outputs:/app/qa_outputs       # bind на хост — видеть результат напрямую
```
...
docker compose exec processing-gateway curl -sS \
    -X POST http://obratka-llm-worker:8000/qa/analyze \
    -H "Content-Type: application/json" \
    --data-binary "@-" < ~/llm-pipeline/input.json
```

Результат пишется в `llm-qa-outputs/<analysis_job_id>/{output_reviews.json,output_summary.json}`.

### 4.3. Когда выключить QA-режим

В проде (после миграции PG на schema 2.0, когда e2e через RabbitMQ заработает) —
**закомментировать** `OBRATKA_QA_ENABLED` и `LLM_QA_OUTPUT_DIR`. Ручка `/qa/analyze`
не имеет аутентификации; пока сервис изолирован в docker-сети — это безопасно,
но если когда-то выставим status-endpoint наружу через nginx — оставлять `/qa`
открытой нельзя.
...
Проверьте, что синк реально прошёл:

```bash
# на VPS
ls -la ~/llm-pipeline/src/obratka/runner.py
...
В compose-override (`docker-compose.with-llm-pipeline.yml`) убедитесь, что
`env_file: - ../llm_pipline/.env` указывает на правильный относительный путь
от `~/processing-gateway/`. На VPS это разрешится в `~/llm-pipeline/.env`
**только** если симлинк есть. Если симлинка нет:

```bash
ln -s ~/llm-pipeline ~/llm_pipline
# либо отредактировать env_file в override на абсолютный путь
```
...
```bash
# 1. RabbitMQ вообще жив?
docker compose ps rabbitmq

# 2. Сеть правильная? Worker должен быть в parser-service_internal
docker inspect obratka-llm-worker --format '{{json .NetworkSettings.Networks}}' | jq

# 3. Creds в .env совпадают с RABBIT_USER/PASS PG-стэка
docker compose exec rabbitmq rabbitmqctl authenticate_user gateway gateway_pwd
# Ожидание: Success
```
...
`rabbitmq:5672`. compose-override перетирает это правильно, но если разворачивали
вручную — проверьте.

### 5.4. «/status/{jobId} возвращает 404 для job-а, который точно был обработан»

Возможные причины:
- Worker рестартанул, а `data/job_state.sqlite` был не в volume (например, изменили
  `LLM_STATE_DB` на путь вне volume) — `/status` теряется
- Volume почистили в §3.3
...
```bash
docker compose logs obratka-llm-worker | grep -E "429|rate" | tail -30
```
...
а LLM публикует 2.0 (см. comment в `docker-compose.with-llm-pipeline.yml`).
Действия:
...
### 5.7. «`docker compose build` падает на `poetry install` — "ImportError LogCmd"»
...
```bash
# Базовый префикс — три compose-файла одной строкой
alias llmc='cd ~/processing-gateway && docker compose -f docker-compose.yml -f docker-compose.full.yml -f docker-compose.with-llm-pipeline.yml'
...
alias llmsh='llmc exec obratka-llm-worker bash'
alias llmhealth='llmc exec obratka-llm-worker python -c "import urllib.request; print(urllib.request.urlopen(\"http://localhost:8000/health/live\").read())"'
alias llmqs='llmc exec rabbitmq rabbitmqctl list_queues name consumers messages_ready | grep llm'
```
