...
/// QA-эндпоинты PG защищены X-Api-Key (`Gateway:ApiKey`). Fixed-time сравнение — чтобы
/// проверка не утекала по timing side-channel. В Development ключ не проверяется (как у Parser-а).
...
public sealed class RequireQaApiKeyAttribute : Attribute, IAuthorizationFilter
{
    ...
    public void OnAuthorization(AuthorizationFilterContext context)
    {
        ...
        {
            context.Result = new ObjectResult(new { error = "QA endpoints disabled: Gateway:ApiKey is not configured" })
            {
