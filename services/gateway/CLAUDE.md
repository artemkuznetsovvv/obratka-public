...
```json
{
  ...
  "additional_context": "Свободный текст пользователя из формы анализа (или null)",
  "reviews": [
    {
      "review_id": <bigint>,
      "text": "...",
...
поля бизнес-контекста компании (категория/подкатегория + «Дополнительный контекст» из формы нового
анализа). Web API читает их из `Company` (`webapi_db`) и шлёт в `StartAnalysisQaRequest` →
`StartAnalysisCommand`; PG хранит снимок в `analysis_jobs.{business_category,business_subcategory,additional_context}`
и вкладывает в `input.json` (`LlmDispatcher`). `schema_version` **не меняется** (`2.0`): по политике
контракта (`llm_pipline/llm_contracts_changed.md` §6) новые поля версию не бампают, а Python-LLM читает
input.json как dict и игнорирует незнакомые ключи. Любое из полей может быть `null`. LLM волен
использовать их как контекст промптов либо игнорировать — обязательными они не являются.
